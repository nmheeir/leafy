...
class JapanesePrompts implements TranslationPromptStrategy {
  @override
  String translateChapter({
    ...
    return '''
You are a professional literary translator, deeply knowledgeable about Japanese language styles and culture.
...
3. Accuracy: Keep proper nouns and jargon as is if the context requires (or transliterate appropriately to Katakana if standard). Ensure the translation flow fits the story context.
...
1. **Literary Quality**: 
   ...
   - Translate freely and expressively, using "expensive" and evocative words to fully recreate the emotions, atmosphere, and voice of the author in Japanese.
   - Prioritize smoothness and naturalness in the target language (Japanese), even if sentence structure needs to change (while keeping the original meaning).
2. **Accuracy**: Preserve proper names and locations (transliterate to Katakana if appropriate).
